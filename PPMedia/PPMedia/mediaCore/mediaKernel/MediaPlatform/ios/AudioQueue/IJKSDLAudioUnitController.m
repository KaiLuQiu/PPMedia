...
- (id)initWithAudioSpec:(const SDL_AudioSpec *)aSpec
{
    ...
    if (self) {
        ...
        if (auComponent == NULL) {
            printf("AudioUnit: AudioComponentFindNext failed");
            self = nil;
            return nil;
        }
        ...
        if (status != noErr) {
            printf("AudioUnit: AudioComponentInstanceNew failed");
            self = nil;
            return nil;
        }
        ...
        status = AudioUnitSetProperty(auUnit,
                                      ...
                                      sizeof(flag));
        if (status != noErr) {
            printf("AudioUnit: failed to set IO mode (%d)", (int)status);
        }
        ...
        status = AudioUnitSetProperty(auUnit,
                                      ...
                                      i_param_size);
        if (status != noErr) {
            printf("AudioUnit: failed to set stream format (%d)", (int)status);
            self = nil;
            return nil;
        }
        ...
        status = AudioUnitGetProperty(auUnit,
                                      ...
                                      &i_param_size);
        if (status != noErr) {
            printf("AudioUnit: failed to verify stream format (%d)\n", (int)status);
        }
        ...
        if (status != noErr) {
            printf("AudioUnit: render callback setup failed (%d)\n", (int)status);
            self = nil;
            return nil;
        }
        ...
        if (status != noErr) {
            printf("AudioUnit: AudioUnitInitialize failed (%d)\n", (int)status);
            self = nil;
            return nil;
        }
...
- (void)pause
{
    ...
    if (status != noErr)
        printf("AudioUnit: failed to stop AudioUnit (%d)\n", (int)status);
}
...
- (void)stop
{
    ...
    OSStatus status = AudioOutputUnitStop(_auUnit);
    if (status != noErr)
        printf("AudioUnit: failed to stop AudioUnit (%d)", (int)status);
}
